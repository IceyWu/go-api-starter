...
type Client struct {
	config       *Config
	envelopeFrom string // SMTP MAIL FROM（纯邮箱地址）
	headerFrom   string // 邮件头 From（可带显示名）
}
...
func NewClient(cfg *Config) *Client {
	...
	if cfg.From != "" {
		if strings.Contains(cfg.From, "@") {
			...
			// From 只填了显示名称，自动拼接
			headerFrom = fmt.Sprintf("%s <%s>", cfg.From, cfg.User)
		}
	}
	if cfg.Port == 0 {
		cfg.Port = 587
...
func (c *Client) sendMailSSL(addr string, to []string, msg []byte) error {
	...
	if err = client.Mail(c.envelopeFrom); err != nil {
