...
func NewFallbackCache(primary, fallback CacheBackend, logger *slog.Logger) *FallbackCache {
	if logger == nil {
	...
	if err := primary.Ping(ctx); err != nil {
		...
		logger.Warn("Primary cache unavailable, starting in fallback mode", "error", err)
	} else {
...
func (f *FallbackCache) checkAndRecover() {
	...
	if err != nil && currentMode == modePrimary {
		...
		f.logger.Warn("Primary cache became unavailable, switching to fallback", "error", err)
...
func (f *FallbackCache) Get(ctx context.Context, key string) ([]byte, error) {
	...
	if err != nil && f.mode.Load() == modePrimary {
		...
		f.mode.Store(modeFallback)
		f.logger.Warn("Primary cache failed, falling back", "error", err)
...
func (f *FallbackCache) Set(ctx context.Context, key string, value []byte, ttl time.Duration) error {
	...
	if err != nil && f.mode.Load() == modePrimary {
		f.mode.Store(modeFallback)
		f.logger.Warn("Primary cache failed on set, falling back", "error", err)
